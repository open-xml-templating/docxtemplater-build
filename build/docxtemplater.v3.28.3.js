...
function expandToOne(postparsed, options) {
  ...
  for (var i = 0, len = postparsed.length; i < len; i++) {
    ...
    if (part.type === "placeholder" && part.module === options.moduleName) {
      try {
        var _getExpandLimit = getExpandLimit(part, i, postparsed, options),
            _getExpandLimit2 = _slicedToArray(_getExpandLimit, 2),
            left = _getExpandLimit2[0],
            right = _getExpandLimit2[1];
